...
#### Update 2026-08-15 — bug 3 fixed at its root, and one retrieval change made

**Bug 3 (duplicate rows) is resolved.** The cause was upstream of the database: `data_extraction/schemes.json` shipped its first 5 records twice — indices 0–4 were stale copies of 5–9, identical in every core field but missing the `application_mode` / `application_links` / `sources_references` enrichment that the other 229 records carry. `extract_schema_llm.py` then inserted both blindly. Worth noting the two copies were *not* identical once ingested: metadata comes from an LLM extraction pass, so the same source text yielded `max_age=40` on one copy of the Girl Child Protection Scheme and `NULL` on the other — meaning which duplicate won the ranking decided whether a citizen was eligible. (The `40` was itself a mis-extraction: the eligibility text's only "40" is *"one of the parents must have undergone sterilization before turning 40"*, a condition on the parent's sterilization age, not an age cap on the applicant — so the `NULL` row was kept.) Fixed at all three layers: 5 rows deleted, `schemes.json` deduped to 229 records, and `UNIQUE(scheme_name)` added to the table plus `ON CONFLICT DO NOTHING` on ingest.

**Bugs 1 and 2 remain open** — both are LLM mis-extractions in the metadata columns (`Pension Scheme - Folk Artist` min_age=60; `Indira Gandhi National Old Age Pension Scheme` flagged women-only). They need a re-extraction pass, not a retrieval change.

**Retrieval:** the p10 MISS above was investigated. Three candidate fixes were measured against the labeled set and rejected — over-fetching a wider SQL pool before re-ranking (9/10 → **6/10**, because `SequenceMatcher` compares a ~200-char query to a ~40-char name and so scores name *length*), replacing the lexical term with token overlap (9/10, worse rank sum), and dropping it entirely (9/10, no hit-rate gain). What shipped instead is a small additive `DISABILITY_AFFINITY_BOOST`: `is_differently_abled` was being used as a SQL gate and then discarded at ranking time, so the corpus's one disability-specific scheme sat below three general pension schemes for a disabled citizen asking for a walking stick. Hit rate held at 9/10, rank sum improved 29 → 26, p10@top_k=10 moved #9 → #6, and all 5 hard negatives plus both blocklists stayed correctly excluded. p10@top_k=3 is still a MISS under every variant tested — the correct scheme is #8 by raw cosine, so it is outside the candidate pool before any re-ranking runs; that needs a better embedding or a query-expansion step, not a re-weighting. Production default is `top_k=10`.
